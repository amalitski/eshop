    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Detail(int id)
        {
            ...
            var categories = _context.Products
                .Where(p => p.Id == id).SelectMany(c => c.CategoryToProducts)
        ...
        public IActionResult AddToCard(int itemId)
        {
            ...
            if (product != null)
            {
                ...
                if (order != null)
                {
                    var orderDetail = _context.OrderDetails.FirstOrDefault(d =>
        ...
        public IActionResult Payment()
        {
            ...
            var res = payment.PaymentRequest($"Payment {order.OrderId}",
                "http://localhost:1635/Home/OnlinePayment/" + order.OrderId, "payment@gmail.com", "09039551666");
            if (res.Result.Status == 100)
            {
                return Redirect("http://localhost/payment" + res.Result.Authority);
            }
            else
            {
                return BadRequest();
